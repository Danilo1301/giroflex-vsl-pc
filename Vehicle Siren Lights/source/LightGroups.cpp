...
LightGroup* LightGroups::CreateLightGroup(int modelId) {
	...
	LightGroup* lightGroup = new LightGroup(modelId, false);
	m_LightGroups[modelId].push_back(lightGroup);
	return lightGroup;
}

LightGroup* LightGroups::CreateLightbarLightGroup(int modelId) {
	Log::file << "[LightGroups] CreateGiroflexLightGroup " << modelId << std::endl;

	LightGroup* lightGroup = new LightGroup(modelId, true);
	m_LightGroups[modelId].push_back(lightGroup);
	return lightGroup;
}
